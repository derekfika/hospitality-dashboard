const CONFIG = {
  ...
  QUOTE_TEMPLATE_DOC_ID: "REPLACE_WITH_MNK_QUOTE_TEMPLATE_DOC_ID",
  QUOTE_ROOT_FOLDER_NAME: "MNK Hospitality",
  SHEET_NAME: "Dashboard Data",
  DASHBOARD_SPREADSHEET_ID: "1GIGIh_oAY0yLrrlXPaSvHte2oMPT8S_dKFAYdZN6nuc",

  PRINTER_EMAIL: "mvt5149ww1d8q0@print.epsonconnect.com",

  DRIVE: {
    ROOT_FOLDER_NAME: "MNK Hospitality"
  },
  ...
  CALENDAR_ID: "mnk@fikacatering.com",
  ...
  DEFAULT_LOCATION: "110 Bishopsgate, EC2N 4AY",
  DEFAULT_FLOOR: "",
...
function getSettingSchema_() {
  return [
    ...
    },
    {
      ...
      access: "admin",
      fields: [
        { key: "ANGEL_COURT_LOGO_URL", label: "MNK logo URL", type: "url", fallback: CONFIG.ANGEL_COURT_LOGO_URL, notes: "Reserved for future branded quote/dashboard use." },
...
function getConfiguredBoolean_(key, fallback) {
  const value = getConfiguredValue_(key, fallback);
  if (value === true || value === false) return value;
  const text = String(value).trim().toUpperCase();
  if (["TRUE", "YES", "Y", "1", "ON"].indexOf(text) !== -1) return true;
  if (["FALSE", "NO", "N", "0", "OFF"].indexOf(text) !== -1) return false;
